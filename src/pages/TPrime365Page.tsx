...
import { Menu, ArrowRight, X, Check, ChevronRight, Syringe, Pill, FlaskConical, Dna, Zap, Dumbbell, Shield, Brain, Flame, Heart, Target, Moon, Bone, Clock, ShieldCheck, Lightbulb, Calendar, AlertCircle, Building2, BadgeCheck, Microscope, Flag, Stethoscope, Lock, Package, Headphones, Droplets, Layers, Baby, DollarSign, Sparkles, ChevronDown } from 'lucide-react';

const SOCIAL_PROOF_MESSAGES = [
  "A man from Texas just started his optimization",
  "TPrime365 shipped to California 2 hours ago",
  "Physician approved — order shipping to Florida",
  "A man from Ohio just started his optimization",
  "TPrime365 shipped to New York 3 hours ago",
  "Physician approved — order shipping to Arizona",
  "A man from Georgia just started his optimization",
  "TPrime365 shipped to Colorado 1 hour ago",
  "Physician approved — order shipping to North Carolina",
  "A man from Pennsylvania just started his optimization",
];
...
const TPrime365Page = () => {
  ...
  useEffect(() => {
    ...
    const showToast = () => {
      ...
      setToastMessage(SOCIAL_PROOF_MESSAGES[index % SOCIAL_PROOF_MESSAGES.length]);
      index++;
  ...
  return (
    <div className="tprime-page">
      ...
      {/* Sticky CTA Bar */}
      <div className={`tprime-sticky-bar ${showStickyBar ? 'visible' : ''}`}>
        <div className="tprime-sticky-bar-inner">
          <span className="tprime-sticky-bar-label"><strong>TPrime365™</strong> — $149/month</span>
          <a href="#process" className="tprime-sticky-bar-btn">
            <span className="sticky-desktop-text">Start Your Optimization <ArrowRight size={14} /></span>
            <span className="sticky-mobile-text">Start Your Optimization — $149/mo <ArrowRight size={14} /></span>
          </a>
        </div>
      </div>

      {/* 3. Hero Section (Enhanced) */}
      <section className={`tprime-hero-section ${!showBanner ? 'no-banner' : ''}`} ref={heroRef}>
        <div className="tprime-hero-container">
          ...
          <div className="tprime-hero-text">
            ...
            <div className="price-row">
              ...
              <span className="price-note">/month — Includes Physician Consultation + Free Shipping</span>
            </div>
            <p className="guarantee-text">If not approved by physician, fully refunded</p>

            {/* Mini Social Proof */}
            <div className="tprime-hero-trust-line">
              <span><Check size={13} /> 50,000+ clients served</span>
              <span><Check size={13} /> FDA-registered 503A pharmacy</span>
              <span><Check size={13} /> 100% money-back guarantee</span>
            </div>

            {/* Urgency */}
            <p className="tprime-urgency-line">
              <Clock size={13} /> {currentMonth} pricing locks in at $149/month — offer subject to change
            </p>

            <div className="tprime-hero-cta-group">
              <AnimatedCTA href="#process">
                Start Your Optimization
                <ArrowRight size={16} />
              </AnimatedCTA>
              <a href="#process" className="tprime-ghost-btn" onClick={(e) => { e.preventDefault(); document.getElementById('process')?.scrollIntoView({ behavior: 'smooth' }); }}>
      ...
      <section className="b365-section" id="process">
        ...
        <div className="tprime-timeline-callout">
          <span><Clock size={16} /> Order to delivery: 7-10 days (if approved)</span>
          <span><ShieldCheck size={16} /> 100% Money-Back Guarantee if physician does not approve</span>
        </div>
      </section>
        ...
        <div className="tprime-problem-grid">
          {[
            ...
            <div className="tprime-problem-card" key={i}>
              <div className="icon-wrap"><card.Icon size={22} /></div>
              <h3>{card.title}</h3>
              <ul>{card.items.map((item, j) => <li key={j}><X size={14} />{item}</li>)}</ul>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="b365-section b365-section-alt" id="science">
        ...
        <div className="tprime-delivery-badge"><span>Sublingual Delivery via MODS Max Gold™</span></div>
          ...
          {[
            ...
            <div className="tprime-ingredient-card" key={i}>
              ...
              {card.sections.map((sec, j) =>
                <div key={j}>
                  <h4>{sec.heading}</h4>
                  <ul>{sec.items.map((item, k) => <li key={k}><ChevronRight size={14} />{item}</li>)}</ul>
                </div>
              )}
            </div>
          )}
      ...
      <section className="b365-section tprime-midpage-cta">
        <h2 className="b365-serif">Ready to <em>Optimize?</em></h2>
        <span className="tprime-midpage-price">$149/month</span>
        <p className="tprime-midpage-sub">Includes everything: 4-in-1 Formula + Physician Consultation + Free Shipping</p>
        <p className="tprime-midpage-guarantee"><ShieldCheck size={14} /> 100% refunded if physician does not approve</p>
        <AnimatedCTA href="#process" className="btn-white-cta">
          Get Started Risk-Free <ArrowRight size={16} />
        </AnimatedCTA>
        <div className="tprime-midpage-trust">
          <span><Check size={13} /> Licensed physician reviews every order</span>
          <span><Check size={13} /> Cancel anytime</span>
          <span><Check size={13} /> Discreet packaging</span>
        </div>
      </section>

      {/* 9. Delivery Advantage */}
      ...
      <section className="b365-section tprime-why-win-strip">
        <h2 className="b365-section-heading b365-serif">Why Men Choose TPrime365™ <em>Over The Rest</em></h2>
        <div className="tprime-win-grid">
          {[
            { Icon: Droplets, title: 'Only Sublingual Delivery', sub: 'Not another oral pill' },
            { Icon: Layers, title: '4-in-1 Formula', sub: 'Competitors sell single-ingredient' },
            { Icon: Baby, title: 'Fertility Preserved', sub: 'TRT destroys it' },
            { Icon: DollarSign, title: '$149 vs $199+', sub: 'More formula for less' },
            { Icon: Sparkles, title: 'Longevity Built In', sub: 'Spermidine + autophagy activation' },
          ].map((card, i) =>
            <div className="tprime-win-card" key={i}>
              <div className="tprime-win-icon"><card.Icon size={22} /></div>
              <h4>{card.title}</h4>
              <p>{card.sub}</p>
            </div>
          )}
        ...
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          <a href="#compare" className="tprime-ghost-btn" onClick={(e) => { e.preventDefault(); document.getElementById('compare')?.scrollIntoView({ behavior: 'smooth' }); }}>
            See Full Comparison <ChevronDown size={14} />
          </a>
        </div>
      </section>
        ...
        <div className="tprime-persona-grid">
          {[
            ...
            <div className="tprime-persona-card" key={i}>
              ...
              <ul>{persona.items.map((item, j) => <li key={j}><ChevronRight size={14} />{item}</li>)}</ul>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="tprime-benefits-grid">
          {[
            ...
            <div className="tprime-benefit-card" key={i}>
              <div className="icon-wrap"><b.Icon size={22} /></div>
              <h4>{b.title}</h4>
              <p>{b.desc}</p>
            </div>
          )}
      ...
      <section className="b365-section tprime-midpage-value">
        ...
        <div style={{ textAlign: 'center', marginTop: 28 }}>
          <AnimatedCTA href="#process">
            Claim Your Formula — $149/mo <ArrowRight size={16} />
          </AnimatedCTA>
          <p className="tprime-midpage-value-guarantee">
            100% refunded if physician does not approve · 60-day satisfaction guarantee · Cancel subscription anytime
          </p>
        </div>
      </section>

      {/* 14. UGC Video Testimonials */}
      <section className="b365-section b365-section-alt">
      ...
      {/* 15. Comparison Table (moved here) */}
      <section className="b365-section" id="compare">
        ...
        <div style={{ textAlign: 'center', marginTop: 28 }}>
          <AnimatedCTA href="#process">
            Start Your Optimization <ArrowRight size={16} />
          </AnimatedCTA>
        </div>
      </section>
      ...
      <section className="b365-section b365-section-alt">
        ...
        <div className="tprime-dosing-card">
          ...
          <div className="tprime-pro-tip">
            <h4><Lightbulb size={16} /> For Best Results:</h4>
            <ul>
        ...
        <div className="tprime-trust-grid">
          {[
            ...
            <div className="tprime-trust-card" key={i}>
              <div className="icon-wrap"><badge.Icon size={22} /></div>
              <h4>{badge.title}</h4>
              <p>{badge.desc}</p>
            </div>
          )}
        ...
        <div className="tprime-safety">
          ...
          <ul>
            {['Have prostate cancer or breast cancer', 'Have severe liver or kidney disease', 'Are under 18 years of age', 'Are allergic to any ingredients', 'Are taking certain medications (physician will review)'].map((item, i) =>
              <li key={i}><AlertCircle size={14} />{item}</li>
            )}
          </ul>
      ...
      {toastMessage && !toastDismissed && (
        <div className="tprime-social-toast">
          <span className="tprime-social-toast-msg">{toastMessage}</span>
          <button className="tprime-social-toast-close" onClick={() => setToastDismissed(true)} aria-label="Dismiss">×</button>
        </div>
      )}
